...
public class ServletHome extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			listEncheres = this.enchmng.selectAllEnchere();
			this.createNewEnchere(listEncheres);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			encheres = this.enchmng.selectAllEnchere();
			int idCategorie = Integer.parseInt(idCat);

			if (idCategorie > 0){
				encheres = this.enchmng.selectByCategories(idCategorie);
			}
			...
			if (session.getAttribute("userIdSessionAttr") != null) {
				...
				if(paramAchat!=null){
					...
					if (isEnchereOuverte !=null){
						listEncheres =  this.enchmng.selectEnchereOuverte();
					} else if (isEncheresEnCours != null) {
						listEncheres = this.enchmng.selectEncheresEnCours(idUser);
					}
					else if(isEncheresRemportees != null){
						listEncheres = this.enchmng.selectEncheresRemportees(idUser);
					}
